...
describe("Given I am connected as an employee", () => {
  ...
  test("Then click on eye icon", () => {
    ...
    const iconEyes = screen.getAllByTestId("icon-eye");
      ...
      const modale = screen.queryByTestId('modalFileEmployee')
      expect(modale).toBeTruthy();
    } 
  });

  test("Then click on the new bill button", async() => {
    document.body.innerHTML = BillsUI({ data: bills });
...
describe("fetch bills from mock API GET", () => {
  describe("When it succees", () => {
    test("Then it should return an array with the corresponding length", async() => {
      ...
      const mockList = await getBills()
    
      expect(getBills).toHaveBeenCalled();
      expect(mockList.length).toBe(4);
    })
  })
